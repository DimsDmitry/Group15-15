...
def neud_level(age):
    ...
    # каждые 2 года разницы от 7 лет превращаются в единицу - вплоть до 15 лет
    n = (min(age, 15) - 7) // 2
    # умножаем каждые 2 года разницы на 1.5, так распределены уровни в таблице
    x = 21 - (1.5 * n)
    return x
...
def ruffier_result(r_index, level):
    ...
    if r_index >= level:
        return 3
    # для всех остальных случаев, если индекс Руфье максимально хороший:
    return 4
...
def test(P1, P2, P3, age):
    ...
    if age < 7:
        return 'Ошибка'
    ruff_index = ruffier_index(P1, P2, P3)  # Производим расчёт
    index = ruffier_result(ruff_index, neud_level(age))
    result = txt_res[index]
    text = txt_index + str(ruff_index) + '\n' + txt_workheart + result
    return text
